...
class DashMovingAverageApp:
    ...
    def __init__(self, df, time_span=365, start_date=None, end_date=None, cache_size=100):
        self.df = df
        ...
        self.available_countries = [col for col in df.columns if col not in ['undl_id', 'date', 'session', 'resolution', 'draft', 'committee_report', 'meeting', 'title', 'agenda_title', 'subjects', 'total_yes', 'total_no', 'total_abstentions', 'total_non_voting', 'total_ms', 'undl_link']]
        
        # Set up caching for expensive calculations
        self.calculate_data = lru_cache(maxsize=cache_size)(self._calculate_data_uncached)
    ...
    def _calculate_data_uncached(self, country1, country2, time_span):
        ...
        try:
            ...
            df_subset['sma'] = df_subset['agreement'].rolling(window=time_span, min_periods=1).mean()
            df_subset['ema'] = df_subset['agreement'].ewm(span=time_span, adjust=False).mean()
    ...
    def setup_layout(self):
        ...
        self.app.layout = html.Div([
            # Header
            html.Div([
                html.H1("🌍 GA Voting Agreement Analysis", 
                       style={'textAlign': 'center', 'color': '#2c3e50', 'marginBottom': '10px'}),
                html.P(f"Interactive analysis of {len(self.available_countries)} countries • True lazy loading with caching",
            ...
            html.Div([
                html.Div([
                    html.Label("Country 1:", style={'fontWeight': 'bold', 'marginBottom': '5px'}),
                    dcc.Dropdown(
                        id='country1-dropdown',
                        options=[{'label': country, 'value': country} for country in self.available_countries],
                ...
                html.Div([
                    html.Label("Country 2:", style={'fontWeight': 'bold', 'marginBottom': '5px'}),
                    dcc.Dropdown(
                        id='country2-dropdown',
                        options=[{'label': country, 'value': country} for country in self.available_countries],
                        value=self.available_countries[1] if len(self.available_countries) > 1 else self.available_countries[0],
                        clearable=False,
                        style={'marginBottom': '15px'}
                    )
                ], style={'width': '30%', 'display': 'inline-block', 'paddingRight': '20px'}),
                ...
                html.Div([
                    html.Label("Time Span (days):", style={'fontWeight': 'bold', 'marginBottom': '5px'}),
                    dcc.Dropdown(
            ...
            html.Div([
                html.Div(id='status-display', style={'padding': '10px', 'backgroundColor': '#d5dbdb', 
...
# Easy setup function
def create_dash_app(df, time_span=365, start_date=None, end_date=None, cache_size=100):
    """
