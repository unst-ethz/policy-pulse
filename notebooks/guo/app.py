...
class DashWordCloudApp:
    ...
    def query_data(self, start_year, end_year, subject_ids : list[str] = None, include_descendants : bool = True) -> pd.DataFrame:
        ...
        return data[['undl_id', 'date', 'title']]
    ...
    def init_wc_data(self):
        ...
        cleanCombinedTitle = pd.read_csv(f"wc_data/undlid_cleanCombinedTitle_20251105_155001.csv")
        data_all = pd.merge(self.query_engine.query_resolutions(), cleanCombinedTitle, on='undl_id', how='left')
        ...
        print("data_all.head(): \n", data_all[['undl_id', 'wc_title']].head())
        ...
        resolution_wc_data = {}
        wc_word_undlid_map = {}
        for undl_id, wc_title in data_all[['undl_id', 'wc_title']].values:
            token_list = self.analyzer.extract_noun_chunks(wc_title) 
            ...
            for token in token_list:
                ...
                if token in ignore_words:
                    continue
                if not all(phrase not in token for phrase in ignore_contain):
                    continue
                if len(token) > 3:
        ...
        for year in range(start_year, end_year + 1):
            ...
            data_filtered = self.query_engine.query_resolutions(
            ...
            # Process titles
            data_filtered['combined_title'] = data_filtered['title'].fillna('') + ' ' + data_filtered['agenda_title'].fillna('')
            data_filtered['combined_title'] = data_filtered['combined_title'].str.strip()
            data_filtered_no_duplicates = data_filtered.drop_duplicates(subset=['combined_title'])
            
            tokens_lists = [
                analyzer.extract_noun_chunks(
                    data_filtered_no_duplicates['combined_title'].iloc[i],
                ) for i in range(len(data_filtered_no_duplicates['combined_title']))
            ]
            ...
            year_data[year] = {
                ...
                'total_resolutions': len(data_filtered_no_duplicates),
                'unique_titles': data_filtered_no_duplicates['combined_title']
            }
    ...
    def setup_callbacks(self):
        ...
        def update_resolution_table(hoverData, start_year, end_year, subject_ids):
            ...
            try:
                ...
                header = html.Tr([
                    html.Th("ID", style={'padding': '6px', 'textAlign': 'left', 'borderBottom': '2px solid #3498db'}),
