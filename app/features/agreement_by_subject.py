...
def register_callbacks(query_engine):
    ...
    def update_subject_agreement(filter_params, selected_c2):
        ...
        if not c1 or not c2:
            msg = (
                "Please select a comparison country in the sidebar settings."
                if c1
                else "Please select a primary country."
            )
        ...
        if df.empty:
            return (
                ...
                {"display": "none"},
                None,
                "No common votes found for the selected criteria (minimum 30 votes per subject required).",
            )
            ...
            [
                ...
                dash_table.DataTable(
                    ...
                    style_data_conditional=[
                        {"if": {"column_id": "agreement_score"}, "textAlign": "right"},
                        {"if": {"column_id": "total_votes"}, "textAlign": "right"},
                    ],
                ),
            ]
        )
...
layout = [
    html.Div(
        ...
        children=[
            html.Span(
                "Comparing with:",
                style={"whiteSpace": "nowrap", "fontSize": "14px", "color": "#555"},
            ),
            dcc.Dropdown(
    ...
    html.Div(
        id="agreement-by-subject-status",
        style={"marginBottom": "10px", "marginTop": "10px", "color": "#666"},
    ),
    dcc.Loading(
        ...
        color="#3498db",
        children=[
            dcc.Graph(id="agreement-by-subject-graph", style={"display": "none"}),
        ],
    ),
