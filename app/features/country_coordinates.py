"""
Country longitude coordinates for map centering.

Maps ISO 3166-1 alpha-3 country codes to their approximate geographic center longitude.
Used to center the choropleth map on the selected country.
"""

# Dictionary mapping ISO alpha-3 country codes to longitude (degrees)
COUNTRY_LONGITUDES = {
...
def get_country_longitude(country_code: str) -> float:
    """
    Get the longitude of a country's geographic center.
    
    Args:
        country_code: ISO 3166-1 alpha-3 country code (e.g., 'USA', 'DEU', 'JPN')
    
    Returns:
        float: Longitude in degrees (-180 to 180). Returns 0.0 if country not found.
    """
    return -COUNTRY_LONGITUDES.get(country_code, 0.0)
